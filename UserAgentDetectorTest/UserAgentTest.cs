...
namespace UserAgentDetectorTest
{
    [TestClass]
    public class UserAgentTest 
    {
        ...
        //public UserAgentTest(IHttpClientFactory clientFactory)
        //{
        ...
        public void runTest()
        {
            for (int i = 0; i < TEST_COUNT; i++)
            {
                try
                {
                    ...
                    Assert.IsTrue(responce.IsSuccessStatusCode, $"\"{responce.RequestMessage}\" has been reterned \"{responce.StatusCode}\"");
                    var resValidate = validate(responce.Content.ReadAsStringAsync().Result, EXPECTED_ARRAY[i]);
                    Assert.IsTrue(resValidate.ok, resValidate.err + $" in test {i + 1}");
                }
        ...
        {
            return _client.GetAsync($"https://localhost:5001/useragent?ua={ua}").Result;
        }
